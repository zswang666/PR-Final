    ...
    with g.as_default():
        ...
        with tf.variable_scope('DATA_LAYER'):
            ...
            label_placeholder = tf.placeholder(tf.int32, [args.batch_size],
                                               'label_placeholder')
            tf.image_summary('input_image', image_placeholder)
        ...
        epts = encoder.vgg16_finetune(image_placeholder, 
                                      ...
                                      reuse=False)
        logits = epts['logits']
        tf.histogram_summary('predictions', epts['predictions'])

        ###################### LOSS & OPT #######################
        with tf.variable_scope('LOSS_N_OPT'):
            # loss
            one_hot_labels = tf.one_hot(label_placeholder, N_CLASSES, axis=-1)
            tf.histogram_summary('one_hot_labels', one_hot_labels)
            #print(logits.get_shape(), one_hot_labels.get_shape())
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits, one_hot_labels),
                                  name='loss')
            tf.scalar_summary('loss', loss)
            # gradients and optimization
            trainables = tf.trainable_variables()
            #for var_i in trainables:
            #    print(var_i.op.name, slim.model_analyzer.tensor_description(var_i))
        ...
        excluded_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'fc8a') + \
                        tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'predictions')
