...
def main(args):
	...
	with tf.Graph().as_default():
		...
		eval_op = tf.equal(tf.argmax(tf.squeeze(logits), 1), tf.cast(label, tf.int64))
		...
		with sess.as_default():
			...
			try:
				while not coord.should_stop():
					...
					_, loss_, eval_op_ = sess.run([train, loss, eval_op], feed_dict=feed_dict)

					# print out loss every x steps
					if step%args.print_step==0:
						print('step {}: loss={:.6}, acc={}'.format(step, loss_, eval_op_))
...
def parse_args(argv):
	...
	parser.add_argument('--trainset', type=str, 
						help='Path to .tfrecords file of training set.', default='mnist_data/mnist_train.tfrecords')
	parser.add_argument('--image_size', type=int, 
						help='Size of (square) images in training set.', default=32)
