	...
	with tf.Graph().as_default():
		...
		with tf.device('/cpu:0'):
			with tf.variable_scope('DataLayer'):
				...
				#								   False)
				real_image, label = cifar10.inputs(None)
		...
		print("Training phase: " + args.train_phase)
		...
		#gen_image = G0
		gen_image = network.generator0(G1, z0, 'G0', reuse=True)
		...
		Q0_loss = tf.reduce_mean(tf.square(recon_z0-z0), name='Q0_loss')
		Q1_loss = tf.reduce_mean(tf.square(recon_z1-z1), name='Q1_loss')
		...
		# G1 total loss
		G1_loss = 2*args.advloss_weight*G1_adv_loss + 3.0*G1_cond_loss + 2*args.entloss_weight*Q1_loss
		...
		# G0 conditional loss
		G0_cond_loss = tf.reduce_mean(tf.square(E0_recon-E0_real), name='G0_cond_loss')
			...
			try:
				while not coord.should_stop():
					...
					if args.train_phase=='INDEPENDENT':
						...
						fetch = [G0_cond_loss, G0_adv_loss, Q0_loss,D0_real_loss, D0_gen_loss,G1_cond_loss, G1_adv_loss, Q1_loss,D1_real_loss, D1_gen_loss]
					...
					if step%args.print_step==0:
						if args.train_phase=='INDEPENDENT':
							print('cond, adv(real,gen), ent, G0D0G1D1')
							print(fetch_)
						else:
					...
					if step%args.save_img_step==0:
						...
						utils.images_on_grid(real_image_,10,10,filename)
						filename = args.img_dir + '/generated_' + str(step) + '.jpg'
						utils.images_on_grid(gen_image_,10,10,filename)		
...
def parse_args(argv):
	...
	parser.add_argument('--trainset', type=str, 
						...
						default='mnist_data/mnist_train.tfrecords')
	...
	parser.add_argument('--condloss_weight', type=float, 
						help='weight for conditional loss', default=0.1)
	...
	parser.add_argument('--pretrained_E', type=str,
 			...
 			default='checkpoints/SGAN_encoder20170110-210625-30000')
	parser.add_argument('--pretrained_model', type=str,
			help='Path to pretrained model.')
	parser.add_argument('--model_save_path', type=str,
			help='Path to save checkpoint models.', default='./checkpoints/train_joint/SGAN')
	parser.add_argument('--summary_step', type=int,
			help='Obtain summary every n steps.', default=10)
	...
	parser.add_argument('--save_img_step', type=int,
			help='Save images every n steps.', default=10)
	parser.add_argument('--img_dir', type=str,
			help='directory to save batches of real and generated images', default='./grid_img/train_independent/')
